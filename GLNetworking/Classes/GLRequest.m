...
- (id)analyResponse:(id)response withTask:(NSURLSessionTask *)task {
    ...
    /* 解码 */
    if (self.obstructDecode == NO && [self._config respondsToSelector:@selector(responseObjectForResponse:data:error:)]) response = [self._config responseObjectForResponse:task.response data:response error:nil];
    /* 尝试 data -> id */
            ...
            case GLMethodGET: {
                ...
                self.task = [self.manager GET:self.url parameters:encodedParam progress:^(NSProgress *_Nonnull downloadProgress) {} success:^(NSURLSessionDataTask *_Nonnull task, id _Nullable responseObject) {
                    ...
                    if (responseObject == nil) {
                        ...
                        userFailure ? kFAD(fadBLK, kErrorCustomUserInfo(userInfo), resp) : kSUC(sucBLK, resp);
                    }
...
//                    无名字类型
            ...
            if (responseObject == nil) {   //返回 空
                ...
                if (jsonError != nil) {    // to dictionary Failed
                    ...
                    userFailure ? kFAD(fadBLK, kErrorCustomUserInfo(customUserInfo), resp) : kSUC(sucBLK, resp);
                }
            }
            dispatch_semaphore_signal(sem);
        } failure:^(NSURLSessionDataTask *_Nullable task, NSError *_Nonnull error) {
